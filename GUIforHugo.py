...
from skimage.filters import threshold_adaptive, threshold_niblack, threshold_sauvola, threshold_yen, threshold_li, threshold_local, threshold_triangle, threshold_otsu
import re
import collections
import pprint
import os
...
class simpleapp_tk(Tkinter.Tk):
    ...
    def initialize(self):  # will be used for creating all the widgets
        ...
        optionList = ["Hybrid", "PrintedBoxesFullSystem", "FindTheSlice", "Adaptive Thresholding", "Binarization Methods Test", "WholeBoxReprocessSA", "WholeBoxWithKNN", "SARemoveIC", "cv2AdaptiveMean", "cv2AdaptiveGaussian", "AccItemCTest", "AdaptiveParameterExperiment", "HybridItemCTest", "AdaptiveWholeBoxTest", "ReprocessSA", "Advanced", "Basic", "ContourGaussianKernelOtsu", "GaussianKernelAndOtsu", "Otsu"]  # add more later
    ...
    def OnButtonClickLoad(self):
        ...
        elif self.method in "FindTheSlice":  # dont need ASN for this
            directory = askdirectory()

            name_list = pickle.load(open("/Users/sigurdandersberg/PycharmProjects/proj1/HWS_all.pickle"))

            for filename in name_list:
                if filename.endswith(".JPG"):
                    processor = img_processor(os.path.join(directory, filename))

                    text, boxes, img, have_iterator = processor.AdaptiveThresholdingExperiment(161, 20)
                    # img_for_slice = 255 - img  # will now be white text on black background
                    im = cv2.imread(os.path.join(directory, filename))
                    # need im to be in numpy format
                    k = 0
                    print img.shape
                    modified_coords = []
                    if boxes:
                        for br in boxes:
                            # now, do adjustments to the boxes
                            cv2.rectangle(im, (0, max(0, br[1]-30)), (img.shape[1], min(br[3]+30, img.shape[0])), (255*(k%2), 255*(k%3), 255*(k%4)), 3+k)
                            # rectangles are drawn correctly

                            modified_coords.append((0, max(0, br[1]-30), img.shape[1], min(br[3]+30, img.shape[0])))
                            cv2.putText(im, str(k+1), (80, int((1.0*min(br[3]+30, img.shape[0])-max(0, br[1]-30))/2)+max(0, br[1]-30)), 1, 8, (255 * (k%2), 255 * (k%3), 255 * (k%4)), 3, cv2.LINE_AA)
                            k += 1
                        # draw the boxes on the img
                        Image.fromarray(im).show()
                        a = raw_input("Enter line number: ")
                        ...
                        if int(a):
                            b = int(a) - 1  # to gt the index number
                            slice = img[modified_coords[b][1]:modified_coords[b][3], 0:img.shape[1]]  # appears to be inconsistent with the im object

                            dir_dest = "/Users/sigurdandersberg/PycharmProjects/proj1/Slices"
                            path_dest = dir_dest + "/Slice_" + filename

                            Image.fromarray(slice).convert("RGB").save(path_dest)
        ...
        elif self.method in "Binarization Methods Test":
            if self.ASN_loaded:

                input = raw_input("1 for Niblack, 2 for Sauvola, 3 for Yen, 4 for Li, 5 for triangle, 6 for local, 7 for Otsu (default), 8 for Hybrid")  # already have Otsu from default tess
                ...
                with open("name_list.pickle",
                          "wb") as f:  # the pickle will get crowded over time if the new lists are written on top of the previous ones
                ...
                # need to check if is printed or not
                for filename in name_list:
                    if self.match_instance.inPrintedList(filename): # don't do if not printed
                        ...
                        if input == "1":
                            img = processor.Niblack()
                        elif input == "2":
                            img = processor.Sauvola()
                        elif input == "3":
                            img = processor.Yen()
                        elif input == "4":
                            img = processor.Li()
                        elif input == "5":
                            img = processor.Triangle()
                        elif input == "6":
                            img = processor.Local()
                        elif input == "7":
                            img = processor.Otsu()  # equivalent to Tesseract default binarization
                        elif input == "8":
                            img = processor.Hybrid()

                        # at this point, img is of type 'uint8', and 255 has been multiplied after conversion from boolean

                        # now, want to do the rest of the pipeline: Noise removal -> Tess -> categorization -> find text lines ..
                        # noise removal

                        # it appears that the below steps are indeed userful for the purpose of noise removal
                        ...
                        for cont in contours:
                            if cv2.contourArea(cont) > area_thresh:
                                large_cnt.append(cont)

                        black_bg = np.zeros((img.shape[0], img.shape[1]), dtype='uint8')

                        cv2.drawContours(black_bg, large_cnt, -1, color=255, thickness=-1)  # use thickness = -1 as in AdaptiveThresholding()
                        # add more below
                        combined = np.logical_and(255-img, 255-black_bg).astype('uint8')*255
                        img_for_tess = Image.fromarray(combined)
                        # img_for_tess = Image.fromarray(img)
                        # combined = img

                        text, boxes = processor.Basic(img_for_tess, combined)
                        ...
                        if have_iterator:
                            img = 255 - combined
                            # text_lines, boxes = processor.Basic(Image.fromarray(img).convert("RGB"), img)
                            # next, feed to Basic, and then display the image output (for now)
                            ...
                            categories, text, itemcode_indeces, unique_ic, skip_segmentation, solidassort_indeces, min_dist_ic, min_dist_sa = self.match_instance.checkerNoText(
                            ...
                            # what is the following actually needed for?
                            """feed = []
                            feed.append(text[categories['Solidcode']])
                            sa_indices_original, dist_original = self.match_instance.saReprocessed(feed,
                                                                                              itemcode_indeces)"""

                            if min_dist_sa < 0.01:  # if perfect by now, then skip everything else, and go directly to the checking of correctness
                                sa_verdict = solidassort_indeces
                                print "Since perfect on SA, we skip further processing!"
                            else:
                                dist_original = min_dist_sa
                                ...
                                # img is in array form.

                                if type(result) is not bool:  # i.e. as long as some kind of change has happened
                                    img_rt = result
                                    Image.fromarray(img_rt).save("binarynotable.png")
                                    proc2 = img_processor("binarynotable.png")
                                    text_rt, boxes_rt, img_rt = proc2.AdaptiveThresholding()  # want to replace this with a different methodd, depending on user selection

                                    text_rt, categories = interpreter.categorizeLines(text_rt)

                                    cat_rt, text_rt, ic_ind_rt, unique_ic_rt, skip_seg_rt, sa_ind_rt, min_dist_ic_rt, min_dist_sa_rt = self.match_instance.checkerNoText(text_rt, categories)
                                    ...
                                    dist_rt = min_dist_sa_rt
                                    # compare the results from this stage to those of the previous method (no removal of table)
                                    if dist_rt < dist_original:  # for now, use the IC guess of whichever image gives best SA score. May want to change that in the future
                                        # sa_indices_original = sa_ind_rt  # then move on from here
                                ...
                                # may not have iterator, in which case the below will lead to TypeError
                                no_iterator = False
                                x_top_left = 0
                                try:
                                    y_top_left = coordinates[1]  # affected by iterator
                                except TypeError:
                                    no_iterator = True
                                ...
                                try:
                                    y_bottom_right = coordinates[3] + int(0.01 * img.shape[0])  # too much?. affected by iterator
                                except TypeError:
                                    no_iterator = True
                                ...
                                if no_iterator:
                                    ...
                                    if (min_dist < 0.25) or (dist_original < 0.25):  # updated 04/09/2017
                                ...
                                if not skip_segmentation:  # updated this part 4/9/17
                                    solidcands = SegementationCleanup.Fujisawa('solidslice.png')
                                    print solidcands

                                    sa_indices_from_segmentation, min_dist_from_segmentation = self.match_instance.saReprocessed(
                                        solidcands, itemcode_indeces)

                                    if (min_dist_first <= min_dist_reprocess) and (min_dist_first <= min_dist_from_segmentation):
                                        sa_verdict = sa_indices_first
                                    elif (min_dist_reprocess <= min_dist_first) and (min_dist_reprocess <= min_dist_from_segmentation):
                            ...
                            print itemcode_indeces
                            print sa_verdict  # i.e. the indices that we go for
                            ...
                            # now, want to return the guesses of item code, solid/assort code and the entire box
                            unique_sa = self.match_instance.uniqueSA(itemcode_indeces, sa_verdict)  # may not be used
                            ...
                            if (correct_index not in itemcode_indeces) or (correct_index not in sa_verdict):
                            ...
                            # if correct_index in itemcode_indeces:  changed 19/4-17: include incorrect also to speed up acc testing
                            ...
                            output_td_vals.append(str(text_guess))
                                # print "Found correct TD"
                            ...
                            print "Correct TD: " + str(count_correct_td) + " "
                            # next, we want to find the guesses for S/A and TD
                            # also, want line-level accuracy for the TD.
                ...
                with open("outputTD.pickle", "w") as f:
                    pickle.dump(output_td_vals, f)

                with open("correctTD.pickle", "w") as f:
                    pickle.dump(correct_td_vals, f)

                with open("outputSA.pickle", "w") as f:
                    pickle.dump(output_sa_vals, f)

                with open("correctSA.pickle", "w") as f:
                    pickle.dump(correct_sa_vals, f)
            ...
            if self.ASN_loaded:
                ...
                for filename in os.listdir(directory):
                    if filename.endswith(".JPG"):
                        ...
                        categories, text, itemcode_indeces, unique_ic, skip_segmentation, solidassort_indeces, min_dist_ic, min_dist_sa = self.match_instance.checkerNoText(text, categories)
        ...
        elif self.method in "PrintedBoxesFullSystem":  # the most up-to-date method as of 04/08/2017 AdaptiveThresholdingExperiment
            block_size = 161
            offset = 20
                ...
                for filename in name_list:
                    if filename.endswith(".JPG"):  # conditions for initiating table removal
                        if self.match_instance.inPrintedList(filename): # don't do if not printed
                            ...
                            processor = img_processor(os.path.join(directory, filename))
                            # text, boxes, img = processor.AdaptiveThresholding()
                            text, boxes, img, have_iterator = processor.AdaptiveThresholdingExperiment(block_size, offset)
                            img_for_slice = 255 - img  # will now be white text on black background
                            # what we want, though, is black text on white bg
                            if have_iterator:
                                interpreter = outputInterpreter()
                                text, categories = interpreter.categorizeLines(text)
                                ...
                                if min_dist_sa < 0.01:  # if perfect by now, then skip everything else, and go directly to the checking of correctness
                                    ...
                                    if type(result) is not bool:  # i.e. as long as some kind of change has happened
                                        ...
                                        Image.fromarray(img_rt).save("binarynotable.png")
                                        proc2 = img_processor("binarynotable.png")
                                        text_rt, boxes_rt, img_rt, have_iterator = proc2.AdaptiveThresholdingExperiment(block_size, offset)
                                    ...
                                    # this part fails when there is no result iterator..but should in general stop everything if there is no iterator, and let the image be incorrect on box level
                                    x_top_left = 0
                                    y_top_left = coordinates[1]
                                    x_bottom_right = img.shape[1]  # assuming x,y are reversed (order) by cv2 image reader.
                                    y_bottom_right = coordinates[3] + int(0.01 * img.shape[0])  # too much?
                                    ...
                                    img_snipped = img_for_slice[y_top_left:y_bottom_right, x_top_left:x_bottom_right]
                                    # img_rgb = cv2.imread(self.filename)
                                    # img_snipped = img_rgb[y_top_left:y_bottom_right, x_top_left:x_bottom_right]  # region may be too small
                                    # Image.fromarray(img_snipped).show()  # then pass it on
                                    #Image.fromarray(img_snipped).show()
                                    Image.fromarray(img_snipped).save("solidslice.png")
                                    ...
                                    # background = np.zeros((dimension_y, dimension_x), dtype='float32')
                                    # background[y_top_left:y_bottom_right, x_top_left:x_bottom_right] = img[y_top_left:y_bottom_right, x_top_left:x_bottom_right]
                                    img_snipped_binary = img_for_slice[y_top_left:y_bottom_right,
                                                         x_top_left:x_bottom_right]  # in numpy array form. Modified 25/4-17
                                    img_snipped_binary_for_tess = Image.fromarray(img_snipped_binary)
                                    ...
                                    if (min_dist < 0.25) or (dist_original < 0.25):  # updated 04/09/2017. Changed from 0.25 to 0.10 on 25 April 2017
                                        skip_segmentation = True
                                    ...
                                    # split into two streams, based on whether segmentation can be skipped or not
                                    if not skip_segmentation:  # updated this part 4/9/17
                                        print "Carrying out segmentation!"
                                        solidcands = SegementationCleanup.Fujisawa('solidslice.png')
                                        print "After performing KNN classification, found these candidates for solidcode: "
                                        print solidcands
                                        count_segmentation += 1
                                        print "Segmentation has been carried out this number of time: " + str(count_segmentation)
...
class img_processor():
    ...
    def Triangle(self):
        img_gs = cv2.imread(self.filename, 0)
        thresh_triangle = threshold_triangle(img_gs)
        return (img_gs > thresh_triangle).astype('uint8')*255

    def Local(self):
        img_gs = cv2.imread(self.filename, 0)
        thresh_local = threshold_local(img_gs, block_size=161, offset=10)
        return (img_gs > thresh_local).astype('uint8')*255


    def Niblack(self):  # local
        img_gs = cv2.imread(self.filename, 0)
        thresh_nib = threshold_niblack(img_gs, window_size=161, k=0.2)
        return (img_gs > thresh_nib).astype('uint8')*255

    def Yen(self):  # global
        img_gs = cv2.imread(self.filename, 0)
        thresh = threshold_yen(img_gs, 256)

        return (img_gs > thresh).astype('uint8')*255

    def Li(self):  # global
        img_gs = cv2.imread(self.filename, 0)
        thresh = threshold_li(img_gs)

        return (img_gs > thresh).astype('uint8')*255

    def Sauvola(self):  # local
        img_gs = cv2.imread(self.filename, 0)
        thresh_sau = threshold_sauvola(img_gs, window_size=161, k=0.2, r=128)
        return (img_gs > thresh_sau).astype('uint8')*255

    def Otsu(self):
        img_gs = cv2.imread(self.filename, 0)
        thresh_otsu = threshold_otsu(img_gs)
        return (img_gs > thresh_otsu).astype('uint8')*255
    ...
    def Hybrid(self):
        ...
        img_gs = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)  # is RGB to gray, not BGR to gray
        ...
        large_contours = []
        for contour in abac:
            if cv2.contourArea(contour) > contour_area_min and (
                cv2.contourArea(contour) < contour_area_max):  # Green's theorem for contour area approximation
        ...
        for i in xrange(0, len(large_contours)):
            ...
            interior_pts = np.where(
                black_bg == 255)  # collection of sets of coordinates corresponding to the interior points of contours
        ...
        t_histo = 200  # number of nonzero pixels in -> direction (threshold value)
        potential_cuts = []
        for x in xrange(1, len(sum1)):
            if (sum1[x] < t_histo) and (sum1[x - 1] >= t_histo):  # cross downward from the left
        ...
        for value in potential_cuts:
            for x in xrange(0,
                            100):  # search for the first local min, with x>value. limit search to 300 vals  may have indexerror with this?
                try:
                    if (sum1[value + x] <= sum1[value + x + 1]) and (
                        sum1[value + x] <= sum1[value + x - 1]):  # i.e. if is local min
        ...
        for seg in zip(segments, coords):  # is in order of increasing y-value
            threshglobal = threshold_li(seg[0])  # Should be an integer? seg[0] refers to the first element, i.e. the segment and not the adjusted cut
            ...
            black_bg[seg[1][0]:seg[1][1], :] = thresholded.astype(
                'uint8') * 255  # need to have the starting y-coordinate, which should be equal to ending of previous segment
        Image.fromarray(black_bg).save("hybrid.png")
...
"""
    ...
    def AdaptiveThresholdingExperiment(self, block_size, offset):
        ...
        offset = offset
        area_lower_bound = 350
        # binar_adaptive = threshold_adaptive(grayscale, block_size, offset=offset
        binar_adaptive = grayscale < threshold_local(grayscale, block_size, offset=offset)
        ...
        combined = np.logical_and(black_bg, noisy)  # why are some tiny pixels left here?
        combined = combined.astype('uint8')*255  # want this to be white text on black bg
        # Image.fromarray(255 - combined).show()
        ...
        return text_output, boxes, (255-combined), have_iterator  # want 255-combined to be black text on white bg
    ...
    def AdaptiveThresholding(self):
        ...
        binar_adaptive = grayscale > threshold_local(grayscale, block_size=block_size, offset=10)
        # next, do noise removal
        noisy = binar_adaptive.astype('uint8') * 255
...
def INSERTION(A, cost=1):  # changed: set costs for inserting hyphen to 0
    if A in "-":
        return 0
    elif A in " ":
        return 0
...
class matcher():
    ...
    def getTDValue(self, index):
        return self.asn[index][0]
    ...
    def saReprocessed(self, text_lines, itemcode_indices):
        ...
        min_dist = 10000
        min_indeces = []
        itemcode_indeces = itemcode_indices  # not used for anything though
    ...
    def saGlobal(self, text_lines):  # does not have modified subs costs
        text = text_lines  # may be one, or more lines
        asn = self.asn

        min_dist = 10000
        min_indices = []

        for text_line in text:  # caution: will fail if the input was just a single line it must be a list
            line = text_line.replace(" ", "")
            line = line.replace("\n", "")
            line = line.upper()
    ...
    def checkerNoText(self, text_lines, ind_dictionary):
        ...
        # handle itemcodes
        if type(ind_dictionary['Itemcode']) is bool and type(result_list_dict['Itemcode']) is bool:  # should never evaluate as true
        ...
        elif type(ind_dictionary['Itemcode']) is not bool and type(result_list_dict['Itemcode']) is bool:  # would never expect this part to be reached
            final_ind_dict['Itemcode'] = ind_dictionary['Itemcode']
        else:
        ...
        if type(ind_dictionary['Solidcode']) is bool and type(result_list_dict['Solidcode']) is bool:
            ...
            final_ind_dict['Solidcode'] = min(ind_dictionary['Solidcode'], result_list_dict['Solidcode'])
        # handle text description. Do have the index for the text description, yet do not return the penalty nor the guessed value
        ...
        if min_dist_ic < dist_threshold:  # overrule the classification made by categorizeLines()
            final_ind_dict['Itemcode'] = result_list_dict['Itemcode']
